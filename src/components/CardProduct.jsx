...
export default function CardProduct({ img, price, title, description, category }) {
    return (
        <div className="w-[320px] h-[520px] bg-white cursor-pointer shadow-xl rounded-md hover:-translate-y-3 transition-all">
            <img src={img} alt="mac" className="rounded-t-lg" />
            <div className="h-1/2 p-5 flex flex-col justify-between">
                <div className="w-full flex flex-col justify-between gap-2">
                    <h1 className="text-xl font-medium">{title}</h1>
                    <span className="bg-slate-300 w-1/3 px-2 rounded-xl text-center text-xs text-slate-600">{category}</span>
                </div>
                ...
                <div className="w-full flex justify-start gap-2">
                    ...
                    <div className="w-10 h-10 flex justify-center items-center rounded-md bg-slate-200 hover:scale-105 transition-all ">
